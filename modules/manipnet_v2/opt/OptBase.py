...
def conv2d(input_, output_dim, k_h=5, k_w=5, d_h=2, d_w=2, stddev=0.02, name="conv2d"):
    with tf.compat.v1.variable_scope(name):
        ...
        conv = tf.nn.conv2d(input_, filters=w, strides=[1, d_h, d_w, 1], padding='SAME')
...
def linear(input_, output_size, scope=None, with_w=False):
    ...
    with tf.compat.v1.variable_scope(scope or "Linear"):
        matrix = tf.compat.v1.get_variable("weights", [shape[-1], output_size], dtype=tf.float32,
...
def LSTM_cell(hidden_size, state_is_tuple=True, activation=tf.nn.relu, keep_prob=[1, 1, 1]):
    ...
    """
    lstm_cell = tf.compat.v1.nn.rnn_cell.BasicLSTMCell(hidden_size, state_is_tuple=state_is_tuple, activation=activation)
...
def linear(input, weights, bias, keep_prob=1):
    input = tf.nn.dropout(input, rate=1 - (keep_prob))
...
def dot_case3(dot, vec1, vec2):
    norm1 = norm_2d(vec1)
    norm2 = norm_2d(vec2)
    angle_abs = tf.math.acos(tf.compat.v1.div(dot, tf.multiply(norm1, norm2)))
...
def loss_log(loss_original, scale):
    ...
    return tf.math.log(loss_original * scale + 1)
...
def angle_rotation_angles(a1, a2):
    rotation = tf.math.floormod(tf.abs(a2 - a1 + tf.constant(2 * np.pi, dtype=tf.float32)),
