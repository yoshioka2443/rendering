...
class MainNN(NeuralNetwork):
    ...
    def BuildModel(self,
        ...
        self.nn_keep_prob_main = tf.compat.v1.placeholder(tf.float32, name=self.name_tensor_keep_prob_main)
        self.nn_keep_prob_encoders = tf.compat.v1.placeholder(tf.float32, [len(dim_encoders)],
                                                    name=self.name_tensor_keep_prob_encoders)
        self.nn_lr_c = tf.compat.v1.placeholder(tf.float32, name='nn_lr_c')
        ...
        if self.num_encoders > 0:
            ...
            for i in range(self.num_encoders):
                ...
                tf.compat.v1.add_to_collection("nn_latent_encoder%0i" % i, output_encoder)
        ...
        tf.compat.v1.add_to_collection("nn_prediction", self.nn_prediction)

        # L2 loss
        self.mse_loss = tf.reduce_mean(tf.square(self.nn_Y - self.nn_prediction))
        # final loss
        self.loss = self.mse_loss
        self.optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=self.nn_lr_c).minimize(self.loss)
    ...
    def Train(self,
        ...
        # Initialize Saver
        saver = tf.compat.v1.train.Saver(max_to_keep=max_save)
        # Start training
