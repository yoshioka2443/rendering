...
# triangle mesh
vertices, indices, uvs, normals = pyredner.generate_sphere(64, 32)
...
scene_args = pyredner.RenderFunction.serialize_scene(\
    ...
    num_samples = 512,
    max_bounces = 1,
    channels = [pyredner.channels.radiance, pyredner.channels.geometry_normal, pyredner.channels.shading_normal]
    )
img = render(202, *scene_args)
geometry_normal = img[..., 3:6]
shading_normal = img[..., 6:]
img = img[..., :3]
...
results = {
    'radiance': img.cpu().detach().numpy(),
    'geometry_normal': geometry_normal.cpu().detach().numpy(),
    'shading_normal': shading_normal.cpu().detach().numpy(),
}
fig, axs = plt.subplots(1, len(results.keys()), figsize=(15, 5))
for ax, (key, val) in zip(axs, results.items()):
    ax.axis('off')
    ax.imshow(val)
    ax.set_title(key)
plt.show()
